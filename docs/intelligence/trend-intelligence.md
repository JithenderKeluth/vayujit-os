...
## 12F Trend ? Winning Product integration

12F is a narrow, immutable, owner-scoped projection of authoritative Trend evidence into a Product Opportunity assessment. It reuses the exact opportunity-linked Trend context first and an exact canonical product-linked context only when unambiguous; it never guesses across owners or contexts. Projection readiness mirrors 12E downstream readiness, while 12E confidence, 12D descriptive momentum/materiality, freshness, contradictions, gaps, hypotheses, and complete evidence lineage remain unchanged.

The projection does not rewrite 9B intelligence, 9E confidence, or 9F scoring, weights, ranking, bands, or decisions. Generic trend direction is not demand, sales, revenue, market size, or success probability. Only a signal definition explicitly typed `DEMAND_INTEREST_PROXY` (or `SEARCH_DEMAND_INTEREST_PROXY`) may be described as a demand-interest proxy; competitor, review, and social signals remain separate evidence domains. Missing, stale, contradictory, ambiguous, or insufficient evidence remains visibly unavailable/research-required. Product Channel, Calendar, Recovery, workers, schedulers, Business Agent, Competitor, and Review engines are not changed.

`TrendWinningProductProjection` is append-only and versioned with an input fingerprint. The scoped API exposes create/current/list/by-id projection reads under `/api/v1/intelligence/product-opportunities/{opportunity_id}/assessments/{assessment_id}/trend-projection`. The Product Opportunity workspace renders a compact Trend evidence panel with readiness, observed signals, historical momentum, confidence, freshness, contradictions, and research gaps using safe Angular interpolation only.


## 12G Trend Business Agent / UX / Operations integration

Trend-enabled Business Agent goals opt in with include_trend_intelligence. The existing 9G runtime adds owner-scoped capabilities for context resolution, evidence use, authoritative 12C analysis, conditional 12D comparison, 12E validation, and optional 12F projection. The plan is dependency ordered and legacy goals retain their existing plan.

The adapter never fabricates observations or provider data. Existing LOCAL_FIXTURE/manual evidence is reused; missing evidence creates a bounded research gap. DISABLED remains fail-closed and LIVE_READ_ONLY is not enabled by 12G. Trend results use the existing 9G attempts, checkpoints, pause/resume, cancel, retry, budgets, artifacts, findings, approvals, and idempotency. No Trend worker, scheduler, recovery action, or second agent runtime is introduced.

Business Agent decision briefs contain a bounded trend_intelligence section with authoritative context, analysis, comparison, validation, and projection IDs, signals, historical momentum, evidence confidence, freshness, agreement, contradictions, readiness, and research gaps. Wording remains evidence-only: observed signal, deterministic Trend intelligence, validated hypothesis, or research gap; it never claims forecast, demand, sales, revenue, market size, or product success and never changes 9E/9F scoring.

Meaningful internal Product Channel audit events are emitted only for validation readiness/research-required states and authoritative moderate/high material changes. Calendar is unchanged, and external writes are always zero. The owner-scoped Operations endpoint is /api/v1/intelligence/business-agent/operations/trend; System Doctor reports Trend registry, lineage, budget, approval, semantic-safety, retry, and external-write hard counters. The Angular Business Agent workspace exposes an Include Trend Intelligence option and bounded Trend step/gap status; the existing Trend workspace remains the navigation surface for context, evidence, analysis, changes, validation, and projection.
