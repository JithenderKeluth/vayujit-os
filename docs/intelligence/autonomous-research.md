...
Slice 5 provides owner-scoped durable research missions with a deterministic local provider. Missions are planned into bounded tasks, persisted with checkpoints, evidence, claims, contradictions, alerts, reports, schedules, and recovery records. External research is deny-by-default and no agent can mutate authority data.
