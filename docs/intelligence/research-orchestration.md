...
The planner maps each mission type to a deterministic task graph. Every task carries a logical role, correlation id, evidence contract, allowed/forbidden source classes, budget, and stop conditions. The worker executes only queued, bounded missions and supports retry-safe idempotency.
