...
Fetch requires HTTPS, an effective allowlist, deny-list precedence, DNS/IP safety, redirect validation, bounded redirects, response bytes, and MIME limits. Only HTML, plain text, and JSON are accepted. Active HTML is reduced to text and marked `UNTRUSTED_EXTERNAL_DATA`; scripts, frames, forms, handlers, and executable content are not trusted.
