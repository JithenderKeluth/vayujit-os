...
`n## Certification evidence`n`nVerification state changes are allowlisted and evidence-gated for `VERIFIED` and `HIGH_CONFIDENCE`; owner-scoped evidence is required. Certification claims and verification observations are append-only and retain historical versions.
