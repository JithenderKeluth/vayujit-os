...
`n## Certification evidence`n`nScore evaluation concurrency is keyed by supplier and model version. Historical scores remain immutable, evidence lineage is retained, risk dimensions are explicit, and mixed currencies are marked non-comparable rather than numerically ranked.
