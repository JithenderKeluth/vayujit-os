...
External AI, search, and web fetch are disabled by default. Approved fetches require HTTP(S), an allowlisted domain, no embedded credentials, standard ports, and no localhost, private, loopback, link-local, reserved, or metadata address. External content remains untrusted data.
