...
@Component({
  selector: 'app-meesho-workspace',
  imports: [FormsModule, RouterLink],
  template: `
    <section class="marketplace-page">
      <header>
        <h1>Meesho operations</h1>
        <p>
          Local fake-certified workspace. Live Meesho validation is
      ...
      <nav class="workspace-tabs" aria-label="Meesho workspace sections">
      ...
      @if (section() === 'Overview' || section() === 'Account') {
        <section class="marketplace-card">
          ...
          @if (!accounts().length) {
            <p class="marketplace-empty">No Meesho account configured.</p>
          }
...
export class MeeshoWorkspaceComponent implements OnInit {
  private readonly http = inject(HttpClient);
  private readonly router = inject(Router);
  private readonly route = inject(ActivatedRoute);
  private readonly base = `${environment.apiUrl}/marketplaces/meesho`;
  ...
  async createListing(): Promise<void> {
    await this.act(async () => {
      ...
      this.selectListing(listing);
      await this.router.navigate(['/marketplaces/listings', listing.id, 'meesho']);
    ...
    await this.act(async () => {
      ...
      await firstValueFrom(
        this.http.put(
          ...
          { variants, idempotency_key: `meesho-variant-matrix-${this.activeListing()!.id}` },
          this.options,
        ),
      );
    ...
    await this.act(async () => {
      ...
      await firstValueFrom(
        this.http.put(
          ...
          {
            variants: [item],
            idempotency_key: `meesho-variant-${this.activeListing()!.id}-${item.stable_variant_key}`,
          },
          this.options,
        ),
      );
      ...
      await firstValueFrom(
        this.http.put(
          `${this.base}/listings/${this.activeListing()!.id}/media`,
          {
            ...
            idempotency_key: `meesho-media-${this.activeListing()!.id}-${this.mediaDraft.media_id}`,
          },
          this.options,
        ),
      );
      ...
      await firstValueFrom(
        this.http.put(
          `${this.base}/listings/${this.activeListing()!.id}/pricing`,
          {
            ...this.priceDraft,
            confirm: true,
            idempotency_key: `meesho-price-${this.activeListing()!.id}-${this.priceDraft.selling_price}`,
          },
          this.options,
        ),
      );
      ...
      const value = await firstValueFrom(
        this.http.post<InventoryState>(
          `${this.base}/listings/${this.activeListing()!.id}/inventory`,
          {
            ...
            confirm: true,
            idempotency_key: `meesho-inventory-${this.activeListing()!.id}-${this.inventoryDraft.quantity}`,
          },
          this.options,
        ),
      );
      ...
      await firstValueFrom(
        this.http.post(
          `${this.base}/listings/${this.activeListing()!.id}/submit`,
          {
            ...
            media: [],
            price: this.priceDraft,
            idempotency_key: `meesho-ui-submit-${this.activeListing()!.id}`,
          },
          this.options,
        ),
      );
  ...
  private async act(action: () => Promise<void>): Promise<void> {
    ...
    try {
      ...
      this.error.set(
        'Unable to complete the Meesho request. Check the safe API error and readiness blockers.',
      );
    }
  }
}
