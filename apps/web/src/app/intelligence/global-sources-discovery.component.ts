...
import {
  GlobalSourcesDiscoveryRequestRecord,
  GlobalSourcesDiscoveryResult,
  GlobalSourcesPreflight,
  IntelligenceService,
} from './intelligence.service';

@Component({
  selector: 'app-global-sources-discovery',
  standalone: true,
  imports: [DatePipe, FormsModule, RouterLink],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <main class="workspace" aria-labelledby="global-sources-title">
      <header class="page-header">
        <div>
          <p class="eyebrow">Intelligence / Supplier discovery</p>
          <h1 id="global-sources-title">Global Sources read-only discovery</h1>
          <p class="lede">
      ...
      @if (loading()) {
        <p role="status" aria-live="polite">Loading Global Sources discoveryâ€¦</p>
      }
      @if (error()) {
      ...
      <section class="panel" aria-labelledby="discover-title">
        ...
        <form (submit)="discover($event)" aria-label="Global Sources discovery request">
          <label
      ...
      <section class="panel" aria-labelledby="history-title">
        ...
        @if (!history().length && !loading()) {
          <p class="empty">No Global Sources discovery runs yet.</p>
        }
        @if (history().length) {
...
export class GlobalSourcesDiscoveryComponent {
  private readonly service = inject(IntelligenceService);
  readonly preflight = signal<GlobalSourcesPreflight | null>(null);
  readonly history = signal<GlobalSourcesDiscoveryRequestRecord[]>([]);
  readonly selectedResults = signal<GlobalSourcesDiscoveryResult[]>([]);
  ...
  async load(): Promise<void> {
    ...
    try {
      const [preflight, history] = await Promise.all([
        this.service.globalSourcesPreflight(),
        this.service.globalSourcesHistory(),
      ]);
      this.preflight.set(preflight);
      this.history.set(history);
    } catch {
      this.error.set(
        'Global Sources discovery is unavailable. Check the authenticated API connection.',
      );
    } finally {
  ...
  async discover(event: Event): Promise<void> {
    ...
    this.error.set('');
    try {
      const response = await this.service.globalSourcesDiscover({
        query: this.query,
  ...
  async select(run: GlobalSourcesDiscoveryRequestRecord): Promise<void> {
    this.loading.set(true);
    this.error.set('');
    try {
      this.selectedResults.set((await this.service.globalSourcesDetail(run.id)).results);
    } catch {
  ...
  claimText(row: GlobalSourcesDiscoveryResult): string {
    const parts = [
