...
import {
  TradeIndiaDiscoveryRequestRecord,
...
@Component({
  selector: 'app-tradeindia-discovery',
  standalone: true,
  imports: [DatePipe, FormsModule, RouterLink],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <main class="workspace" aria-labelledby="tradeindia-title">
      <header class="page-header">
        <div>
          <p class="eyebrow">Intelligence / Supplier discovery</p>
          <h1 id="tradeindia-title">TradeIndia read-only discovery</h1>
          <p class="lede">
      ...
      <section class="panel" aria-labelledby="discover-title">
        ...
        <form (submit)="discover($event)" aria-label="TradeIndia discovery request">
          <label
      ...
      <section class="panel" aria-labelledby="history-title">
        ...
        @if (!history().length && !loading()) {
          <p class="empty">No TradeIndia discovery runs yet.</p>
        }
        @if (history().length) {
...
export class TradeIndiaDiscoveryComponent {
  private readonly service = inject(IntelligenceService);
  readonly preflight = signal<TradeIndiaPreflight | null>(null);
  readonly history = signal<TradeIndiaDiscoveryRequestRecord[]>([]);
  readonly selectedResults = signal<TradeIndiaDiscoveryResult[]>([]);
  ...
  async load(): Promise<void> {
    ...
    try {
      const [preflight, history] = await Promise.all([
        this.service.tradeindiaPreflight(),
        this.service.tradeindiaHistory(),
      ]);
      this.preflight.set(preflight);
      this.history.set(history);
    } catch {
      this.error.set(
        'TradeIndia discovery is unavailable. Check the authenticated API connection.',
      );
    } finally {
  ...
  async discover(event: Event): Promise<void> {
    ...
    this.error.set('');
    try {
      const response = await this.service.tradeindiaDiscover({
        query: this.query,
  ...
  async select(run: TradeIndiaDiscoveryRequestRecord): Promise<void> {
    this.loading.set(true);
    this.error.set('');
    try {
      this.selectedResults.set((await this.service.tradeindiaDetail(run.id)).results);
    } catch {
  ...
  claimText(row: TradeIndiaDiscoveryResult): string {
    const parts = [
