...
import {
  IndiaMartDiscoveryRequestRecord,
...
@Component({
  selector: 'app-indiamart-discovery',
  standalone: true,
  imports: [DatePipe, FormsModule, RouterLink],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <main class="workspace" aria-labelledby="indiamart-title">
      <header class="page-header">
        <div>
          <p class="eyebrow">Intelligence / Supplier discovery</p>
          <h1 id="indiamart-title">IndiaMART read-only discovery</h1>
          <p class="lede">
      ...
      @if (loading()) {
        <p role="status" aria-live="polite">Loading IndiaMART discovery…</p>
      }
      @if (error()) {
      ...
      <section class="panel" aria-labelledby="discover-title">
        ...
        <form (submit)="discover($event)" aria-label="IndiaMART discovery request">
          <label
      ...
      <section class="panel" aria-labelledby="history-title">
        ...
        @if (!history().length && !loading()) {
          <p class="empty">No IndiaMART discovery runs yet.</p>
        }
        @if (history().length) {
...
export class IndiaMartDiscoveryComponent {
  private readonly service = inject(IntelligenceService);
  readonly preflight = signal<IndiaMartPreflight | null>(null);
  readonly history = signal<IndiaMartDiscoveryRequestRecord[]>([]);
  readonly selectedResults = signal<IndiaMartDiscoveryResult[]>([]);
  ...
  async load(): Promise<void> {
    ...
    try {
      const [preflight, history] = await Promise.all([
        this.service.indiamartPreflight(),
        this.service.indiamartHistory(),
      ]);
      this.preflight.set(preflight);
      this.history.set(history);
    } catch {
      this.error.set('IndiaMART discovery is unavailable. Check the authenticated API connection.');
    } finally {
  ...
  async discover(event: Event): Promise<void> {
    ...
    this.error.set('');
    try {
      const response = await this.service.indiamartDiscover({
        query: this.query,
  ...
  async select(run: IndiaMartDiscoveryRequestRecord): Promise<void> {
    this.loading.set(true);
    this.error.set('');
    try {
      this.selectedResults.set((await this.service.indiamartDetail(run.id)).results);
    } catch {
  ...
  claimText(row: IndiaMartDiscoveryResult): string {
    const parts = [
