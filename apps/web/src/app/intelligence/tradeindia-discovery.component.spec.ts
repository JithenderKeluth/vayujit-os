...
import { TradeIndiaDiscoveryComponent } from './tradeindia-discovery.component';
import { IntelligenceService } from './intelligence.service';

describe('TradeIndiaDiscoveryComponent', () => {
  function configure(service: Partial<IntelligenceService>) {
    return TestBed.configureTestingModule({
      imports: [TradeIndiaDiscoveryComponent],
  ...
  it('renders readiness, history, discovery-only results, and claims', async () => {
    ...
    const fixture = TestBed.createComponent(TradeIndiaDiscoveryComponent);
    await fixture.whenStable();
    fixture.detectChanges();

    expect(fixture.nativeElement.textContent).toContain('TradeIndia read-only discovery');
  ...
  it('shows the safe error state when the authenticated API is unavailable', async () => {
    await configure({
      tradeindiaPreflight: () => Promise.reject(new Error('offline')),
      tradeindiaHistory: () => Promise.reject(new Error('offline')),
    });
    const fixture = TestBed.createComponent(TradeIndiaDiscoveryComponent);
    await fixture.whenStable();
    fixture.detectChanges();
  ...
  it('keeps landmarks, labels, captions, alerts, and keyboard controls accessible', async () => {
    ...
    expect(root.querySelector('main[aria-labelledby="tradeindia-title"]')).not.toBeNull();
    expect(root.querySelectorAll('h1')).toHaveLength(1);
    expect(root.querySelector('form[aria-label="TradeIndia discovery request"]')).not.toBeNull();
  ...
  it('defines bounded table scrolling and mobile breakpoints for 390, 768, and 1280 layouts', () => {
    const source = String(TradeIndiaDiscoveryComponent);
  ...
  it('covers the final accessibility assertions', async () => {
    await configure({
      tradeindiaPreflight: () => new Promise(() => undefined),
      tradeindiaHistory: () => Promise.resolve([]),
    });
    const fixture = TestBed.createComponent(TradeIndiaDiscoveryComponent);
    fixture.detectChanges();
    const root = fixture.nativeElement as HTMLElement;
