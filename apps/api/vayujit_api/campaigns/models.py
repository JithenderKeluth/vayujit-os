...
class CampaignActivity(Base):
    ...
    row_version: Mapped[int] = mapped_column(Integer, default=1)
    replaces_activity_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("campaign_activities.id", ondelete="RESTRICT"),
        unique=True,
        index=True,
    )
    ...
    replacement_created_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
