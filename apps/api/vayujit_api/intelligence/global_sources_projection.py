"""Small owner-scoped projections for the GlobalSources discovery boundary."""
...
def integrity(db: Session, owner: User) -> dict[str, object]:
    request_duplicate_groups = db.execute(
        select(GlobalSourcesDiscoveryRequest.idempotency_key, func.count())
        .where(GlobalSourcesDiscoveryRequest.owner_id == owner.id)
        .group_by(GlobalSourcesDiscoveryRequest.idempotency_key)
        .having(func.count() > 1)
    ).all()
    result_duplicate_groups = db.execute(
        select(GlobalSourcesDiscoveryResult.provider_result_id, func.count())
        .where(GlobalSourcesDiscoveryResult.owner_id == owner.id)
        .group_by(GlobalSourcesDiscoveryResult.provider_result_id)
        .having(func.count() > 1)
    ).all()
    ...
    orphan_results = int(
        db.scalar(
            ...
            .outerjoin(
                GlobalSourcesDiscoveryRequest,
                GlobalSourcesDiscoveryRequest.id == GlobalSourcesDiscoveryResult.request_id,
            )
            .where(
                GlobalSourcesDiscoveryResult.owner_id == owner.id,
                GlobalSourcesDiscoveryRequest.id.is_(None),
            )
        )
        or 0
    )
    ...
    return {
        ...
        "storage": {
            "intelligence_global_sources_discovery_requests": requests,
            "intelligence_global_sources_discovery_results": results,
            **autonomous_counts,
        },
        "duplicates": {
...
def product_channel(db: Session, owner: User, product_id: uuid.UUID) -> dict[str, object]:
    ...
    rows = list(
        db.scalars(
            ...
            .where(
                GlobalSourcesDiscoveryResult.owner_id == owner.id,
                GlobalSourcesDiscoveryResult.product_id == product_id,
            )
            .order_by(GlobalSourcesDiscoveryResult.retrieved_at.desc())
        )
    )
...
def storage_inventory() -> dict[str, object]:
    return {
        "tables": [
            "intelligence_global_sources_discovery_requests",
            "intelligence_global_sources_discovery_results",
    ...
    executions = list(
        db.scalars(
            select(MarketplaceExecution).where(
                ...
                MarketplaceExecution.provider == "GLOBAL_SOURCES",
            )
        )
    )
    windows = list(
        db.scalars(
            select(MarketplaceRateWindow).where(
                ...
                MarketplaceRateWindow.provider == "GLOBAL_SOURCES",
            )
        )
    )
    return {
        ...
        "preflight": {
            ...
            "enabled": settings.global_sources_enabled,
            "kill_switch": settings.global_sources_kill_switch,
