# ruff: noqa: E501
"""TradeIndia discovery orchestration over normalized supplier intelligence."""
...
from vayujit_api.intelligence.tradeindia import (
    TradeIndiaListing,
    discover_local,
    provider_preflight,
)
from vayujit_api.intelligence.tradeindia_models import (
    TradeIndiaDiscoveryRequest,
    TradeIndiaDiscoveryResult,
)
...
def preflight(settings: Settings) -> dict[str, object]:
    ...
    if settings.tradeindia_mode == "LIVE_READ_ONLY":
        value = {**value, "status": "BLOCKED_BY_EXTERNAL_CONFIGURATION"}
    return value


class _TradeIndiaAdapter:
    """Adapt the read-only TradeIndia provider to the shared runtime contract."""
    ...
    def normalize(self, payload: object) -> list[dict[str, object]]:
        if not isinstance(payload, list) or not all(
            ...
            raise ValueError("TradeIndia provider returned an invalid response")
...
def _listing(value: Mapping[str, object]) -> TradeIndiaListing:
    """Convert the runtime JSON-safe payload to the normalized provider type."""
    metadata_value = value.get("metadata")
    return TradeIndiaListing(
...
def _append_observations(row: TradeIndiaDiscoveryResult, listing: object, stamp: datetime) -> None:
    values = {
...
def _payload(row: TradeIndiaDiscoveryResult) -> dict[str, object]:
    return {
        "id": str(row.id),
...
def discover(
    ...
    *,
    ...
    request = db.scalar(
        select(TradeIndiaDiscoveryRequest).where(
            TradeIndiaDiscoveryRequest.owner_id == owner.id,
            TradeIndiaDiscoveryRequest.idempotency_key == key,
        )
    )
    if request is not None:
        existing_rows = list(
            db.scalars(
                select(TradeIndiaDiscoveryResult)
                .where(TradeIndiaDiscoveryResult.request_id == request.id)
                .order_by(TradeIndiaDiscoveryResult.created_at)
            )
        )
        return {
    ...
    if readiness["status"] != "READY":
        raise HTTPException(409, f"TradeIndia discovery is {readiness['status']}.")
    if result_limit > settings.tradeindia_max_results:
        raise HTTPException(422, "TradeIndia result limit exceeds the configured safety bound.")
    now = _now()
    day_count = (
        db.scalar(
            select(func.count())
            .select_from(TradeIndiaDiscoveryRequest)
            .where(
                TradeIndiaDiscoveryRequest.owner_id == owner.id,
                TradeIndiaDiscoveryRequest.created_at >= now - timedelta(days=1),
            )
        )
        or 0
    )
    if int(day_count) >= settings.tradeindia_daily_quota:
        raise HTTPException(429, "TradeIndia daily quota reached; retry later.")
    stamp = now
    runtime_result = execute_marketplace_lifecycle(
        db,
        owner,
        _TradeIndiaAdapter(settings, result_limit=result_limit, country_code=country_code),
        provider="TRADEINDIA",
        ...
        identity_key=key,
        query=query,
        mode=settings.tradeindia_mode,
        requests_per_minute=settings.tradeindia_requests_per_minute,
        requests_per_hour=settings.tradeindia_requests_per_hour,
    )
    if runtime_result.status != "SUCCEEDED":
        raise HTTPException(409, "TradeIndia discovery could not be completed safely.")
    execution = db.scalar(
    ...
    if execution is None:
        raise HTTPException(409, "TradeIndia discovery execution is unavailable.")
    listings = _TradeIndiaAdapter(
    ...
    stamp = _now()
    request = TradeIndiaDiscoveryRequest(
        owner_id=owner.id,
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="tradeindia.discovery.requested",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    rows: list[TradeIndiaDiscoveryResult] = []
    for payload in listings:
        ...
        source = db.scalar(
            select(SupplierSource).where(
                ...
                SupplierSource.source_type == "tradeindia",
        ...
        if offering is None:
            ...
            db.add(offering)
            db.flush()
        evidence_key = f"tradeindia:{owner.id}:{listing.provider_result_id}:{hashlib.sha256(listing.listing_name.encode()).hexdigest()[:16]}"
        evidence = db.scalar(
        ...
        if evidence is None:
            evidence = SupplierEvidence(
                ...
                reference=listing.provider_result_id,
                source_url=listing.source_url,
                normalized_value={"provider": "TRADEINDIA", "listing": listing.listing_name},
        ...
        existing_result = db.scalar(
            select(TradeIndiaDiscoveryResult).where(
                TradeIndiaDiscoveryResult.owner_id == owner.id,
                TradeIndiaDiscoveryResult.provider_result_id == listing.provider_result_id,
            )
        )
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="tradeindia.discovery.completed",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    db.commit()
...
def _request_payload(row: TradeIndiaDiscoveryRequest) -> dict[str, object]:
    return {
        "id": str(row.id),
        "provider": row.provider,
        "mode": row.mode,
...
def list_discoveries(db: Session, owner: User, limit: int = 100) -> list[dict[str, object]]:
    rows = list(
        db.scalars(
            select(TradeIndiaDiscoveryRequest)
            .where(TradeIndiaDiscoveryRequest.owner_id == owner.id)
            .order_by(TradeIndiaDiscoveryRequest.created_at.desc())
            .limit(limit)
        )
    )
...
def detail(db: Session, owner: User, request_id: uuid.UUID) -> dict[str, object]:
    row = db.scalar(
        select(TradeIndiaDiscoveryRequest).where(
            TradeIndiaDiscoveryRequest.id == request_id,
            TradeIndiaDiscoveryRequest.owner_id == owner.id,
        )
    )
    if row is None:
        raise HTTPException(404, "TradeIndia discovery not found.")
    results = list(
        db.scalars(
            select(TradeIndiaDiscoveryResult)
            .where(TradeIndiaDiscoveryResult.request_id == row.id)
            .order_by(TradeIndiaDiscoveryResult.created_at)
        )
    )
...
def operations(db: Session, owner: User, settings: Settings) -> dict[str, object]:
    return {
        ...
        "budget": {
            "requests_per_minute": settings.tradeindia_requests_per_minute,
            "requests_per_hour": settings.tradeindia_requests_per_hour,
            "daily_quota": settings.tradeindia_daily_quota,
            "max_results": settings.tradeindia_max_results,
            "retry_max_attempts": settings.tradeindia_retry_max_attempts,
        },
        "recovery": {
            "registered": True,
        ...
        "result_count": int(
            db.scalar(
                select(func.count())
                .select_from(TradeIndiaDiscoveryResult)
                .where(TradeIndiaDiscoveryResult.owner_id == owner.id)
            )
            or 0
        ),
        "failure_count": int(
            db.scalar(
                ...
                .where(
                    TradeIndiaDiscoveryRequest.owner_id == owner.id,
                    TradeIndiaDiscoveryRequest.status == "failed",
                )
            )
            or 0
        ),
...
def handoff_evidence(
    ...
    result = db.scalar(
        select(TradeIndiaDiscoveryResult).where(
            TradeIndiaDiscoveryResult.id == result_id,
            TradeIndiaDiscoveryResult.owner_id == owner.id,
        )
    )
    if result is None:
        raise HTTPException(404, "TradeIndia discovery result not found.")
    mission = db.scalar(
    ...
    retrieval_identity = (
        f"tradeindia:{owner.id}:{result.provider_result_id}:{observation_fingerprint}"
    )
    evidence = db.scalar(
    ...
    if evidence is not None and (evidence.mission_id != mission.id or evidence.task_id != task.id):
        raise HTTPException(409, "TradeIndia evidence is already linked to another research task.")
    if evidence is None:
        ...
        normalized = {
            ...
            "claims": claims,
            "text": "Normalized TradeIndia discovery observation; independent verification required.",
            "source_profile": "TRADEINDIA_LOCAL_FIXTURE",
        ...
        record_event(
            db,
            actor_id=owner.id,
            action="tradeindia.discovery.evidence_handoff",
            entity_type=result.__tablename__,
            entity_id=result.id,
            metadata={"verification_status": evidence.verification_status},
            idempotency_key=f"tradeindia:evidence:{retrieval_identity}",
        )
        db.commit()
    else:
        result.evidence_id = evidence.id
