...
from vayujit_api.intelligence.alibaba_schemas import (
    AlibabaDiscoveryRequest,
    AlibabaEvidenceHandoffRequest,
)
from vayujit_api.intelligence.alibaba_service import (
    detail,
    discover,
    handoff_evidence,
    list_discoveries,
...
router = APIRouter(prefix="/api/v1/intelligence/alibaba", tags=["alibaba-discovery"])
...
def run_discovery(data: AlibabaDiscoveryRequest, db: DB, owner: Owner) -> dict[str, object]:
    return discover(
        db,
        owner,
        get_settings(),
...
def discovery_evidence_handoff(
    result_id: uuid.UUID,
    data: AlibabaEvidenceHandoffRequest,
    db: DB,
    owner: Owner,
) -> dict[str, object]:
