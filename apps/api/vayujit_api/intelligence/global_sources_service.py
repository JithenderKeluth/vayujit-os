# ruff: noqa: E501
"""GlobalSources discovery orchestration over normalized supplier intelligence."""
...
from vayujit_api.intelligence.global_sources import (
    GlobalSourcesListing,
    discover_local,
    provider_preflight,
)
from vayujit_api.intelligence.global_sources_models import (
    GlobalSourcesDiscoveryRequest,
    GlobalSourcesDiscoveryResult,
)
...
def preflight(settings: Settings) -> dict[str, object]:
    ...
    if settings.global_sources_mode == "LIVE_READ_ONLY":
        value = {**value, "status": "BLOCKED_BY_EXTERNAL_CONFIGURATION"}
    return value


class _GlobalSourcesAdapter:
    """Adapt the read-only GlobalSources provider to the shared runtime contract."""
    ...
    def normalize(self, payload: object) -> list[dict[str, object]]:
        if not isinstance(payload, list) or not all(
            ...
            raise ValueError("GlobalSources provider returned an invalid response")
...
def _listing(value: Mapping[str, object]) -> GlobalSourcesListing:
    """Convert the runtime JSON-safe payload to the normalized provider type."""
    metadata_value = value.get("metadata")
    return GlobalSourcesListing(
...
def _append_observations(
    row: GlobalSourcesDiscoveryResult, listing: object, stamp: datetime
) -> None:
    values = {
...
def _payload(row: GlobalSourcesDiscoveryResult) -> dict[str, object]:
    return {
        "id": str(row.id),
...
def discover(
    ...
    *,
    ...
    request = db.scalar(
        select(GlobalSourcesDiscoveryRequest).where(
            GlobalSourcesDiscoveryRequest.owner_id == owner.id,
            GlobalSourcesDiscoveryRequest.idempotency_key == key,
        )
    )
    if request is not None:
        existing_rows = list(
            db.scalars(
                select(GlobalSourcesDiscoveryResult)
                .where(GlobalSourcesDiscoveryResult.request_id == request.id)
                .order_by(GlobalSourcesDiscoveryResult.created_at)
            )
        )
        return {
    ...
    if readiness["status"] != "READY":
        raise HTTPException(409, f"GlobalSources discovery is {readiness['status']}.")
    if result_limit > settings.global_sources_max_results:
        raise HTTPException(422, "GlobalSources result limit exceeds the configured safety bound.")
    now = _now()
    day_count = (
        db.scalar(
            select(func.count())
            ...
            .where(
                GlobalSourcesDiscoveryRequest.owner_id == owner.id,
                GlobalSourcesDiscoveryRequest.created_at >= now - timedelta(days=1),
            )
        )
        or 0
    )
    if int(day_count) >= settings.global_sources_daily_quota:
        raise HTTPException(429, "GlobalSources daily quota reached; retry later.")
    stamp = now
    runtime_result = execute_marketplace_lifecycle(
        db,
        owner,
        _GlobalSourcesAdapter(settings, result_limit=result_limit, country_code=country_code),
        ...
        requests_per_minute=settings.global_sources_requests_per_minute,
        requests_per_hour=settings.global_sources_requests_per_hour,
    )
    if runtime_result.status != "SUCCEEDED":
        raise HTTPException(409, "GlobalSources discovery could not be completed safely.")
    execution = db.scalar(
    ...
    if execution is None:
        raise HTTPException(409, "GlobalSources discovery execution is unavailable.")
    listings = _GlobalSourcesAdapter(
        settings, result_limit=result_limit, country_code=country_code
    ).normalize(execution.provider_payload)
    # Provider retries may repeat a result; persist each logical listing once.
    seen_provider_ids: set[str] = set()
    deduplicated_listings: list[dict[str, object]] = []
    for value in listings:
        provider_result_id = str(value.get("provider_result_id"))
        if provider_result_id in seen_provider_ids:
            continue
        seen_provider_ids.add(provider_result_id)
        deduplicated_listings.append(value)
    ...
    stamp = _now()
    request = GlobalSourcesDiscoveryRequest(
        owner_id=owner.id,
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="global_sources.discovery.requested",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    rows: list[GlobalSourcesDiscoveryResult] = []
    for payload in listings:
        ...
        if supplier is None:
            supplier = Supplier(
                ...
                source_identity="global_sources",
        ...
        source = db.scalar(
            select(SupplierSource).where(
                ...
                SupplierSource.source_type == "global_sources",
        ...
        if offering is None:
            ...
            db.add(offering)
            db.flush()
        evidence_key = f"global_sources:{owner.id}:{listing.provider_result_id}:{hashlib.sha256(listing.listing_name.encode()).hexdigest()[:16]}"
        evidence = db.scalar(
        ...
        if evidence is None:
            evidence = SupplierEvidence(
                ...
                reference=listing.provider_result_id,
                source_url=listing.source_url,
                normalized_value={"provider": "GLOBAL_SOURCES", "listing": listing.listing_name},
        ...
        existing_result = db.scalar(
            select(GlobalSourcesDiscoveryResult).where(
                GlobalSourcesDiscoveryResult.owner_id == owner.id,
                GlobalSourcesDiscoveryResult.provider_result_id == listing.provider_result_id,
            )
        )
        if existing_result is not None:
            ...
            continue
        row = GlobalSourcesDiscoveryResult(
            owner_id=owner.id,
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="global_sources.discovery.completed",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    db.commit()
...
def _request_payload(row: GlobalSourcesDiscoveryRequest) -> dict[str, object]:
    return {
        "id": str(row.id),
        "provider": row.provider,
        "mode": row.mode,
...
def list_discoveries(db: Session, owner: User, limit: int = 100) -> list[dict[str, object]]:
    rows = list(
        db.scalars(
            select(GlobalSourcesDiscoveryRequest)
            .where(GlobalSourcesDiscoveryRequest.owner_id == owner.id)
            .order_by(GlobalSourcesDiscoveryRequest.created_at.desc())
            .limit(limit)
        )
    )
...
def detail(db: Session, owner: User, request_id: uuid.UUID) -> dict[str, object]:
    row = db.scalar(
        select(GlobalSourcesDiscoveryRequest).where(
            GlobalSourcesDiscoveryRequest.id == request_id,
            GlobalSourcesDiscoveryRequest.owner_id == owner.id,
        )
    )
    if row is None:
        raise HTTPException(404, "GlobalSources discovery not found.")
    results = list(
        db.scalars(
            select(GlobalSourcesDiscoveryResult)
            .where(GlobalSourcesDiscoveryResult.request_id == row.id)
            .order_by(GlobalSourcesDiscoveryResult.created_at)
        )
    )
...
def operations(db: Session, owner: User, settings: Settings) -> dict[str, object]:
    return {
        ...
        "budget": {
            "requests_per_minute": settings.global_sources_requests_per_minute,
            "requests_per_hour": settings.global_sources_requests_per_hour,
            "daily_quota": settings.global_sources_daily_quota,
            "max_results": settings.global_sources_max_results,
            "retry_max_attempts": settings.global_sources_retry_max_attempts,
        },
        "recovery": {
            "registered": True,
        ...
        "result_count": int(
            db.scalar(
                select(func.count())
                .select_from(GlobalSourcesDiscoveryResult)
                .where(GlobalSourcesDiscoveryResult.owner_id == owner.id)
            )
            or 0
        ),
        "failure_count": int(
            db.scalar(
                ...
                .where(
                    GlobalSourcesDiscoveryRequest.owner_id == owner.id,
                    GlobalSourcesDiscoveryRequest.status == "failed",
                )
            )
            or 0
        ),
...
def handoff_evidence(
    ...
    result = db.scalar(
        select(GlobalSourcesDiscoveryResult).where(
            GlobalSourcesDiscoveryResult.id == result_id,
            GlobalSourcesDiscoveryResult.owner_id == owner.id,
        )
    )
    if result is None:
        raise HTTPException(404, "GlobalSources discovery result not found.")
    mission = db.scalar(
    ...
    retrieval_identity = (
        f"global_sources:{owner.id}:{result.provider_result_id}:{observation_fingerprint}"
    )
    evidence = db.scalar(
    ...
    if evidence is not None and (evidence.mission_id != mission.id or evidence.task_id != task.id):
        raise HTTPException(
            409, "GlobalSources evidence is already linked to another research task."
        )
    if evidence is None:
        ...
        normalized = {
            ...
            "claims": claims,
            "text": "Normalized GlobalSources discovery observation; independent verification required.",
            "source_profile": "GLOBAL_SOURCES_LOCAL_FIXTURE",
        ...
        record_event(
            db,
            actor_id=owner.id,
            action="global_sources.discovery.evidence_handoff",
            entity_type=result.__tablename__,
            entity_id=result.id,
            metadata={"verification_status": evidence.verification_status},
            idempotency_key=f"global_sources:evidence:{retrieval_identity}",
        )
        db.commit()
    else:
        result.evidence_id = evidence.id
