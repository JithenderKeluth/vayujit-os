...
from vayujit_api.intelligence.global_sources_schemas import (
    GlobalSourcesDiscoveryRequest,
    GlobalSourcesEvidenceHandoffRequest,
)
from vayujit_api.intelligence.global_sources_service import (
    detail,
    discover,
    handoff_evidence,
    list_discoveries,
...
router = APIRouter(prefix="/api/v1/intelligence/global_sources", tags=["global_sources-discovery"])
...
def run_discovery(data: GlobalSourcesDiscoveryRequest, db: DB, owner: Owner) -> dict[str, object]:
    return discover(
        db,
        owner,
        get_settings(),
...
def discovery_evidence_handoff(
    result_id: uuid.UUID,
    data: GlobalSourcesEvidenceHandoffRequest,
    db: DB,
    owner: Owner,
) -> dict[str, object]:
