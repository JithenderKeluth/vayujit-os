"""Small owner-scoped projections for the IndiaMART discovery boundary."""
...
def integrity(db: Session, owner: User) -> dict[str, object]:
    request_duplicate_groups = db.execute(
        select(IndiaMartDiscoveryRequest.idempotency_key, func.count())
        .where(IndiaMartDiscoveryRequest.owner_id == owner.id)
        .group_by(IndiaMartDiscoveryRequest.idempotency_key)
        .having(func.count() > 1)
    ).all()
    result_duplicate_groups = db.execute(
        select(IndiaMartDiscoveryResult.provider_result_id, func.count())
        .where(IndiaMartDiscoveryResult.owner_id == owner.id)
        .group_by(IndiaMartDiscoveryResult.provider_result_id)
        .having(func.count() > 1)
    ).all()
    ...
    orphan_results = int(
        db.scalar(
            ...
            .outerjoin(
                IndiaMartDiscoveryRequest,
                IndiaMartDiscoveryRequest.id == IndiaMartDiscoveryResult.request_id,
            )
            .where(
                IndiaMartDiscoveryResult.owner_id == owner.id,
                IndiaMartDiscoveryRequest.id.is_(None),
            )
        )
        or 0
    )
    ...
    return {
        ...
        "storage": {
            "intelligence_indiamart_discovery_requests": requests,
            "intelligence_indiamart_discovery_results": results,
            **autonomous_counts,
        },
        "duplicates": {
...
def product_channel(db: Session, owner: User, product_id: uuid.UUID) -> dict[str, object]:
    ...
    rows = list(
        db.scalars(
            ...
            .where(
                IndiaMartDiscoveryResult.owner_id == owner.id,
                IndiaMartDiscoveryResult.product_id == product_id,
            )
            .order_by(IndiaMartDiscoveryResult.retrieved_at.desc())
        )
    )
...
def operational_summary(db: Session, owner: User, settings: Settings) -> dict[str, object]:
    ...
    return {
        ...
        "preflight": {
            ...
            "enabled": settings.indiamart_enabled,
            "kill_switch": settings.indiamart_kill_switch,
