"""Durable TradeIndia read-only discovery records."""
...
class TradeIndiaDiscoveryRequest(Base):
    __tablename__ = "intelligence_tradeindia_discovery_requests"
    __table_args__ = (
        UniqueConstraint(
    ...
    provider: Mapped[str] = mapped_column(String(32), default="TRADEINDIA")
...
class TradeIndiaDiscoveryResult(Base):
    __tablename__ = "intelligence_tradeindia_discovery_results"
    __table_args__ = (
        UniqueConstraint(
    ...
    request_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("intelligence_tradeindia_discovery_requests.id", ondelete="CASCADE"),
        index=True,
    )
