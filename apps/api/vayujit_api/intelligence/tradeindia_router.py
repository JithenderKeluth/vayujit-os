...
from vayujit_api.intelligence.tradeindia_schemas import (
    TradeIndiaDiscoveryRequest,
    TradeIndiaEvidenceHandoffRequest,
)
from vayujit_api.intelligence.tradeindia_service import (
    detail,
    discover,
    handoff_evidence,
    list_discoveries,
...
router = APIRouter(prefix="/api/v1/intelligence/tradeindia", tags=["tradeindia-discovery"])
...
def run_discovery(data: TradeIndiaDiscoveryRequest, db: DB, owner: Owner) -> dict[str, object]:
    return discover(
        db,
        owner,
        get_settings(),
...
def discovery_evidence_handoff(
    result_id: uuid.UUID,
    data: TradeIndiaEvidenceHandoffRequest,
    db: DB,
    owner: Owner,
) -> dict[str, object]:
