# ruff: noqa: E501
"""Alibaba discovery orchestration over normalized supplier intelligence."""
...
from vayujit_api.intelligence.alibaba import (
    AlibabaListing,
    discover_local,
    provider_preflight,
)
...
def preflight(settings: Settings) -> dict[str, object]:
    ...
    if settings.alibaba_mode == "LIVE_READ_ONLY":
        value = {**value, "status": "BLOCKED_BY_EXTERNAL_CONFIGURATION"}
    return value


class _AlibabaAdapter:
    """Adapt the read-only Alibaba provider to the shared runtime contract."""
    ...
    def normalize(self, payload: object) -> list[dict[str, object]]:
        if not isinstance(payload, list) or not all(
            ...
            raise ValueError("Alibaba provider returned an invalid response")
...
def _listing(value: Mapping[str, object]) -> AlibabaListing:
...
def _append_observations(row: AlibabaDiscoveryResult, listing: object, stamp: datetime) -> None:
    values = {
...
def _payload(row: AlibabaDiscoveryResult) -> dict[str, object]:
    return {
        "id": str(row.id),
...
def discover(
    ...
    *,
    ...
    request = db.scalar(
        select(AlibabaDiscoveryRequest).where(
            AlibabaDiscoveryRequest.owner_id == owner.id,
            AlibabaDiscoveryRequest.idempotency_key == key,
        )
    )
    if request is not None:
        existing_rows = list(
            db.scalars(
                ...
                .where(AlibabaDiscoveryResult.request_id == request.id)
                .order_by(AlibabaDiscoveryResult.created_at)
            )
        )
        return {
    ...
    if readiness["status"] != "READY":
        raise HTTPException(409, f"Alibaba discovery is {readiness['status']}.")
    if result_limit > settings.alibaba_max_results:
        raise HTTPException(422, "Alibaba result limit exceeds the configured safety bound.")
    now = _now()
    day_count = (
        db.scalar(
            select(func.count())
            ...
            .where(
                AlibabaDiscoveryRequest.owner_id == owner.id,
                AlibabaDiscoveryRequest.created_at >= now - timedelta(days=1),
            )
        )
        or 0
    )
    if int(day_count) >= settings.alibaba_daily_quota:
        raise HTTPException(429, "Alibaba daily quota reached; retry later.")
    stamp = now
    runtime_result = execute_marketplace_lifecycle(
        db,
        owner,
        _AlibabaAdapter(settings, result_limit=result_limit, country_code=country_code),
        provider="ALIBABA",
    ...
    if runtime_result.status != "SUCCEEDED":
        raise HTTPException(409, "Alibaba discovery could not be completed safely.")
    execution = db.scalar(
    ...
    if execution is None:
        raise HTTPException(409, "Alibaba discovery execution is unavailable.")
    listings = _AlibabaAdapter(
    ...
    stamp = _now()
    request = AlibabaDiscoveryRequest(
        owner_id=owner.id,
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="alibaba.discovery.requested",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    rows: list[AlibabaDiscoveryResult] = []
    for payload in listings:
        ...
        source = db.scalar(
            select(SupplierSource).where(
                ...
                SupplierSource.source_type == "alibaba",
        ...
        if offering is None:
            ...
            db.add(offering)
            db.flush()
        evidence_key = f"alibaba:{owner.id}:{listing.provider_result_id}:{hashlib.sha256(listing.listing_name.encode()).hexdigest()[:16]}"
        evidence = db.scalar(
        ...
        if evidence is None:
            evidence = SupplierEvidence(
                ...
                reference=listing.provider_result_id,
                source_url=listing.source_url,
                normalized_value={"provider": "ALIBABA", "listing": listing.listing_name},
        ...
        existing_result = db.scalar(
            select(AlibabaDiscoveryResult).where(
                AlibabaDiscoveryResult.owner_id == owner.id,
                AlibabaDiscoveryResult.provider_result_id == listing.provider_result_id,
            )
        )
    ...
    record_event(
        db,
        actor_id=owner.id,
        action="alibaba.discovery.completed",
        entity_type=request.__tablename__,
        entity_id=request.id,
    )
    db.commit()
...
def _request_payload(row: AlibabaDiscoveryRequest) -> dict[str, object]:
    return {
        "id": str(row.id),
        "provider": row.provider,
        "mode": row.mode,
...
def detail(db: Session, owner: User, request_id: uuid.UUID) -> dict[str, object]:
    row = db.scalar(
        select(AlibabaDiscoveryRequest).where(
            AlibabaDiscoveryRequest.id == request_id,
            AlibabaDiscoveryRequest.owner_id == owner.id,
        )
    )
    if row is None:
        raise HTTPException(404, "Alibaba discovery not found.")
    results = list(
        db.scalars(
            select(AlibabaDiscoveryResult)
            .where(AlibabaDiscoveryResult.request_id == row.id)
            .order_by(AlibabaDiscoveryResult.created_at)
        )
    )
...
def handoff_evidence(
    ...
    result = db.scalar(
        select(AlibabaDiscoveryResult).where(
            AlibabaDiscoveryResult.id == result_id,
            AlibabaDiscoveryResult.owner_id == owner.id,
        )
    )
    if result is None:
        raise HTTPException(404, "Alibaba discovery result not found.")
    mission = db.scalar(
    ...
    retrieval_identity = f"alibaba:{owner.id}:{result.provider_result_id}:{observation_fingerprint}"
    evidence = db.scalar(
    ...
    if evidence is not None and (evidence.mission_id != mission.id or evidence.task_id != task.id):
        raise HTTPException(409, "Alibaba evidence is already linked to another research task.")
    if evidence is None:
        ...
        normalized = {
            ...
            "claims": claims,
            "text": "Normalized Alibaba discovery observation; independent verification required.",
            "source_profile": "ALIBABA_LOCAL_FIXTURE",
        ...
        record_event(
            db,
            actor_id=owner.id,
            action="alibaba.discovery.evidence_handoff",
            entity_type=result.__tablename__,
            entity_id=result.id,
            metadata={"verification_status": evidence.verification_status},
            idempotency_key=f"alibaba:evidence:{retrieval_identity}",
        )
        db.commit()
    else:
        result.evidence_id = evidence.id
