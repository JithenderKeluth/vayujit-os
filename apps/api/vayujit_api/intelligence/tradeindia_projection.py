"""Small owner-scoped projections for the TradeIndia discovery boundary."""
...
def integrity(db: Session, owner: User) -> dict[str, object]:
    request_duplicate_groups = db.execute(
        select(TradeIndiaDiscoveryRequest.idempotency_key, func.count())
        .where(TradeIndiaDiscoveryRequest.owner_id == owner.id)
        .group_by(TradeIndiaDiscoveryRequest.idempotency_key)
        .having(func.count() > 1)
    ).all()
    result_duplicate_groups = db.execute(
        select(TradeIndiaDiscoveryResult.provider_result_id, func.count())
        .where(TradeIndiaDiscoveryResult.owner_id == owner.id)
        .group_by(TradeIndiaDiscoveryResult.provider_result_id)
        .having(func.count() > 1)
    ).all()
    ...
    orphan_results = int(
        db.scalar(
            ...
            .outerjoin(
                TradeIndiaDiscoveryRequest,
                TradeIndiaDiscoveryRequest.id == TradeIndiaDiscoveryResult.request_id,
            )
            .where(
                TradeIndiaDiscoveryResult.owner_id == owner.id,
                TradeIndiaDiscoveryRequest.id.is_(None),
            )
        )
        or 0
    )
    ...
    return {
        ...
        "storage": {
            "intelligence_tradeindia_discovery_requests": requests,
            "intelligence_tradeindia_discovery_results": results,
            **autonomous_counts,
        },
        "duplicates": {
...
def product_channel(db: Session, owner: User, product_id: uuid.UUID) -> dict[str, object]:
    ...
    rows = list(
        db.scalars(
            ...
            .where(
                TradeIndiaDiscoveryResult.owner_id == owner.id,
                TradeIndiaDiscoveryResult.product_id == product_id,
            )
            .order_by(TradeIndiaDiscoveryResult.retrieved_at.desc())
        )
    )
    ...
    executions = list(
        db.scalars(
            select(MarketplaceExecution).where(
                ...
                MarketplaceExecution.provider == "TRADEINDIA",
            )
        )
    )
    windows = list(
        db.scalars(
            select(MarketplaceRateWindow).where(
                ...
                MarketplaceRateWindow.provider == "TRADEINDIA",
            )
        )
    )
    return {
        ...
        "preflight": {
            ...
            "enabled": settings.tradeindia_enabled,
            "kill_switch": settings.tradeindia_kill_switch,
