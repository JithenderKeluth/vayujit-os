"""Durable normalized projections for the deterministic Flipkart boundary.
        ...
        if existing_item is None:
            db.add(
                MarketplaceOrderItem(
                    ...
                    variant_id=None,
                    marketplace_sku=sku,
                    title_snapshot=str(item.get("title", "Flipkart order item"))[:500],
                    quantity=quantity,
                    unit_price=unit,
...
def import_order_events(
    db: Session, account: MarketplaceAccount, connector: FlipkartCommerceConnector
) -> dict[str, int]:
    ...
    for value in connector.get_cancellations():
        ...
        if existing_cancellation is None:
            ...
            existing_cancellation = MarketplaceCancellation(
                ...
                safe_notes="Imported from deterministic Flipkart boundary.",
                created_at=stamp,
                updated_at=stamp,
            )
    ...
    for value in connector.get_refunds():
        ...
        if exists is None:
            ...
            exists = MarketplaceRefund(
                ...
                currency=str(value.get("currency", "INR"))[:3].upper(),
                reason=str(value.get("reason", "Flipkart refund"))[:240],
        ...
        for fee in fee_items:
            ...
            if existing is None:
                existing = MarketplaceFee(
                    ...
                    description="Flipkart normalized financial event",
                    occurred_at=stamp,
                    created_at=stamp,
                    updated_at=stamp,
                )
            ...
            if line is None:
                line = MarketplaceSettlementLine(
                    ...
                    amount=-amount,
                    currency=settlement.currency,
                    description="Flipkart normalized fee line",
                    created_at=stamp,
                    updated_at=stamp,
                )
                db.add(line)
