"""Owner-scoped Meesho Commerce API.
...
live Meesho compatibility is deliberately not claimed.
"""
...
from vayujit_api.commerce.meesho_projection import (
...
router = APIRouter(prefix="/api/v1/marketplaces/meesho", tags=["meesho"])
...
def _account(db: Session, user: User, account_id: uuid.UUID) -> MarketplaceAccount:
    row = db.scalar(
        select(MarketplaceAccount).where(
            ...
            MarketplaceAccount.marketplace == "meesho",
        )
    )
    if row is None:
        raise HTTPException(404, "Meesho account was not found.")
    return row


def _connector(account: MarketplaceAccount) -> MeeshoCommerceConnector:
    return MeeshoCommerceConnector(seller_id=account.seller_account_id)


def _require_enabled(account: MarketplaceAccount) -> None:
    if not account.enabled:
        raise HTTPException(409, "Meesho account is disabled; enable it before remote mutations.")
...
def _account_response(row: MarketplaceAccount) -> dict[str, object]:
    return {
        ...
        "marketplace_id": MEESHO_MARKETPLACE.marketplace_id,
        "currency": MEESHO_MARKETPLACE.currency,
        "locale": MEESHO_MARKETPLACE.locale,
        "capabilities": [
            "categories",
            "attributes",
            "listing_preview",
...
def list_accounts(db: DatabaseSession, user: Owner) -> list[dict[str, object]]:
    rows = db.scalars(
        ...
        .where(MarketplaceAccount.owner_id == user.id, MarketplaceAccount.marketplace == "meesho")
...
def create_account(data: AccountCreate, db: DatabaseSession, user: Owner) -> dict[str, object]:
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            409, "A Meesho account with these identifiers already exists."
        ) from error
    record_event(
        db,
        actor_id=user.id,
        action="meesho.account_configured",
        entity_type="marketplace_account",
        entity_id=row.id,
        metadata={"marketplace": "meesho", "environment": row.environment},
    )
    db.commit()
...
def validate_account(account_id: uuid.UUID, db: DatabaseSession, user: Owner) -> dict[str, object]:
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.account_validated",
        entity_type="marketplace_account",
        entity_id=row.id,
...
def enable_account(
    ...
    if row.validation_status != "valid":
        raise HTTPException(409, "Validate the Meesho account before enabling it.")
    row.enabled = True
    row.updated_at = now()
    db.commit()
    return _account_response(row)
...
def remove_credential(
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.account_credential_removed",
        entity_type="marketplace_account",
        entity_id=row.id,
        metadata={},
    )
...
def list_listings(
    ...
    query = select(MarketplaceListing).where(
        MarketplaceListing.owner_id == user.id, MarketplaceListing.marketplace == "meesho"
    )
    if status:
...
def create_listing(data: ListingCreate, db: DatabaseSession, user: Owner) -> dict[str, object]:
    ...
    if artifact is None or artifact.status != "approved":
        raise HTTPException(
            409, "Only an approved content artifact can be attached to a Meesho listing."
        )
    stamp = now()
...
def _listing(db: Session, user: User, listing_id: uuid.UUID) -> MarketplaceListing:
    row = db.scalar(
        select(MarketplaceListing).where(
            ...
            MarketplaceListing.marketplace == "meesho",
        )
    )
    if row is None:
        raise HTTPException(404, "Meesho listing was not found.")
    return row
...
def _readiness(
    ...
    if not account.enabled:
        block("account_disabled", "Enable the Meesho account before submission.", "account")
    if account.credential_status != "configured":
        block("credential_missing", "A stored Meesho credential is required.", "credential")
    if account.validation_status != "valid":
        block("account_not_validated", "Validate the Meesho account before submission.", "account")
    ...
    if not listing.category:
        block("category_missing", "Select a Meesho category.", "category")
    ...
    for issue in MeeshoVariantPolicy.validate(variant_payload):
...
def preview(
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.listing_previewed",
        entity_type="marketplace_listing",
        entity_id=row.id,
...
def submit(
    ...
    if not ready["ready"]:
        raise HTTPException(409, "Meesho listing is not ready for submission.")
    ...
    if result.status == "throttled":
        raise HTTPException(429, "Meesho throttled the listing submission; retry safely.")
    if result.remote_id:
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.listing_submitted",
        entity_type="marketplace_listing",
        entity_id=row.id,
...
def reconcile(listing_id: uuid.UUID, db: DatabaseSession, user: Owner) -> dict[str, object]:
    ...
    if not row.remote_listing_id:
        raise HTTPException(404, "Meesho listing remote identifier was not found.")
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.listing_reconciled",
        entity_type="marketplace_listing",
        entity_id=row.id,
...
def save_variants(
    ...
    row = _listing(db, user, listing_id)
    issues = MeeshoVariantPolicy.validate(data.variants)
    if issues:
...
def save_price(
    ...
    _require_enabled(account)
    if not data.confirm:
        raise HTTPException(409, "Confirm the Meesho price update.")
    issues = MeeshoPricingPolicy.validate(
        {
            "selling_price": data.selling_price,
            "list_price": data.list_price,
            "currency": data.currency,
        },
        MEESHO_MARKETPLACE.currency,
    )
    if issues:
        raise HTTPException(422, issues[0].message)
    if not row.remote_listing_id:
        raise HTTPException(409, "Submit the listing before changing its Meesho price.")
    ...
    if operation_status == "throttled":
        raise HTTPException(429, "Meesho throttled the price update; retry safely.")
    if operation_status in {"unknown", "rejected"}:
        raise HTTPException(409, "Meesho price update requires reconciliation.")
    old = db.scalar(
        select(MarketplacePrice).where(
            MarketplacePrice.owner_id == user.id,
            MarketplacePrice.listing_id == row.id,
...
def update_inventory(
    ...
    _require_enabled(account)
    if not data.confirm:
        raise HTTPException(409, "Confirm the Meesho inventory update.")
    ...
    operation_status = str(remote.get("operation_status", "succeeded"))
    if operation_status == "throttled":
        raise HTTPException(429, "Meesho throttled the inventory update; retry safely.")
    if operation_status in {"unknown", "rejected"}:
        raise HTTPException(409, "Meesho inventory update requires reconciliation.")
    stamp = now()
    item = db.scalar(
        select(MarketplaceInventory).where(
            MarketplaceInventory.owner_id == user.id,
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.inventory_updated",
        entity_type="marketplace_listing",
        entity_id=row.id,
...
def import_financials(account_id: uuid.UUID, db: DatabaseSession, user: Owner) -> dict[str, int]:
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.settlements_imported",
        entity_type="marketplace_account",
        entity_id=account.id,
        metadata=cast(dict[str, object], counts),
    )
...
def list_orders(account_id: uuid.UUID, db: DatabaseSession, user: Owner) -> list[dict[str, object]]:
    ...
    for order in orders:
        ...
        result.append(
            {
                "id": order.id,
                "meesho_order_id": order.remote_order_id,
                "status": order.status,
...
def _meesho_drift(listing_id: uuid.UUID, db: Session, user: User) -> dict[str, object]:
...
def drift(listing_id: uuid.UUID, db: DatabaseSession, user: Owner) -> dict[str, object]:
    return _meesho_drift(listing_id, db, user)
...
def review_drift(listing_id: uuid.UUID, db: DatabaseSession, user: Owner) -> dict[str, object]:
    result = _meesho_drift(listing_id, db, user)
    record_event(
        db,
        actor_id=user.id,
        action="meesho.drift_reviewed",
        entity_type="marketplace_listing",
...
def keep_remote_drift(
    ...
    if not data.confirm:
        raise HTTPException(400, "Explicit confirmation is required to keep remote Meesho changes.")
    result = _meesho_drift(listing_id, db, user)
    from vayujit_api.commerce.models import MarketplaceDriftRecord
    ...
    record_event(
        db,
        actor_id=user.id,
        action="meesho.drift_kept_remote",
        entity_type="marketplace_listing",
        entity_id=listing_id,
...
def overwrite_drift(
    ...
    if not data.confirm:
        raise HTTPException(400, "Explicit confirmation is required for Meesho drift overwrite.")
    listing = db.scalar(
        select(MarketplaceListing).where(
            MarketplaceListing.id == listing_id,
            MarketplaceListing.owner_id == user.id,
    ...
    if not result.get("status"):
        raise HTTPException(502, "Meesho overwrite did not complete safely.")
    listing.drift_state = "none"
    record_event(
        db,
        actor_id=user.id,
        action="meesho.drift_overwritten",
        entity_type="marketplace_listing",
