"""add GlobalSources discovery tables"""
...
revision = "20261019_0098"
down_revision = "20261018_0097"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "owner_id", "idempotency_key", name="uq_global_sources_discovery_request_idempotency"
        ),
    )
    op.create_index(
        "ix_global_sources_discovery_requests_owner_id",
        "intelligence_global_sources_discovery_requests",
        ["owner_id"],
    )
    op.create_index(
        "ix_global_sources_discovery_requests_status",
        "intelligence_global_sources_discovery_requests",
        ["status"],
    )
    op.create_index(
        "ix_global_sources_discovery_requests_marketplace_execution_id",
        "intelligence_global_sources_discovery_requests",
        ["marketplace_execution_id"],
    )
    op.create_table(
        ...
        sa.Column(
            "request_id",
            uid,
            sa.ForeignKey("intelligence_global_sources_discovery_requests.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
        sa.UniqueConstraint(
            "owner_id", "provider_result_id", name="uq_global_sources_discovery_result_identity"
        ),
        sa.UniqueConstraint(
            "owner_id",
            "request_id",
            "idempotency_key",
            name="uq_global_sources_result_request_idempotency",
        ),
    )
    op.create_index(
        "ix_global_sources_discovery_results_owner_id",
        "intelligence_global_sources_discovery_results",
        ["owner_id"],
    )
    op.create_index(
        "ix_global_sources_discovery_results_request_id",
        "intelligence_global_sources_discovery_results",
        ["request_id"],
    )
    op.create_index(
        "ix_global_sources_discovery_results_retrieved_at",
        "intelligence_global_sources_discovery_results",
        ["retrieved_at"],
    )
...
def downgrade() -> None:
    op.drop_table("intelligence_global_sources_discovery_results")
    op.drop_table("intelligence_global_sources_discovery_requests")
