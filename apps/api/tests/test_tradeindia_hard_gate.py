...
from vayujit_api.intelligence import tradeindia_service
...
from vayujit_api.intelligence.tradeindia_models import TradeIndiaDiscoveryRequest
...
def test_tradeindia_retry_matrix(case: str, code: str, expected: bool) -> None:
...
def test_tradeindia_rate_and_retry_budget_matrix(client: TestClient) -> None:
    ...
    with test_ai_integration.factory() as db:
        ...
        available = consume_rate_window(
            db, owner, "TRADEINDIA", requests_per_minute=2, requests_per_hour=2
        )
        assert available.remaining == 1
        consume_rate_window(db, owner, "TRADEINDIA", requests_per_minute=2, requests_per_hour=2)
        ...
        row = MarketplaceExecution(
            owner_id=owner.id, provider="TRADEINDIA", correlation_id="budget", identity_key="budget"
        )
        db.add(row)
        db.flush()
...
def test_tradeindia_crash_checkpoint_recovery_is_duplicate_free(
    ...
    with test_ai_integration.factory() as db:
        ...
        assert owner is not None
        calls = 0
        adapter = tradeindia_service._TradeIndiaAdapter(
            Settings(tradeindia_enabled=True, tradeindia_mode="LOCAL_FIXTURE"),
            result_limit=2,
            country_code="IN",
        )
...
def test_tradeindia_concurrency_and_repeatability(client: TestClient) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    ...
    def run_once(suffix: str) -> tuple[str, int]:
        ...
        def invoke(_: int) -> str:
            with factory() as db:
                ...
                assert owner is not None
                barrier.wait()
                result = tradeindia_service.discover(
                    db,
                    owner,
                    Settings(tradeindia_enabled=True, tradeindia_mode="LOCAL_FIXTURE"),
        ...
        with factory() as db:
            count = (
                db.scalar(
                    ...
                    .where(TradeIndiaDiscoveryRequest.idempotency_key == f"concurrent-{suffix}")
                )
                or 0
            )
...
def _evidence_pair(
    ...
    right = AutonomousResearchEvidence(
        ...
        task_id=task.id,
        source_class="SUPPLIER",
        source_reference=f"tradeindia-{key}",
        retrieval_identity=f"tradeindia:{key}",
        content_type="application/json",
        ...
        freshness_status="FRESH",
        verification_reason="fixture",
        source_profile="tradeindia-local",
        provider="TRADEINDIA",
        canonical_url="https://www.tradeindia.com/source",
...
def test_tradeindia_contradiction_matrix_is_replay_safe(client: TestClient, key: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_tradeindia_change_matrix_and_replay(client: TestClient, field: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_tradeindia_alert_matrix_and_replay(client: TestClient, alert_type: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
        ...
        first = record_external_alert(
            ...
            title=f"TradeIndia {alert_type}",
            detail="Review required",
            identity=f"tradeindia:{alert_type}",
        ...
        later = record_external_alert(
            ...
            identity=f"tradeindia:{alert_type}:later",
            severity="REQUIRES_REVIEW",
        )
        db.commit()
...
def test_tradeindia_rejected_data_has_zero_delta(client: TestClient, state: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_tradeindia_canonical_e2e_lineage_storage_and_replay(client: TestClient) -> None:
    context = setup_context(client)
    payload = {
        "query": "canonical tradeindia bottle",
        "product_id": context["product"]["id"],
        "result_limit": 2,
        "idempotency_key": "tradeindia-canonical",
    }
    ...
    with test_ai_integration.factory() as db:
        ...
        assert execution.provider == "TRADEINDIA" and execution.status == "SUCCEEDED"
        assert (
            execution.lineage
...
def test_tradeindia_security_privacy_and_xss_boundaries(client: TestClient) -> None:
    context = setup_context(client)
    ...
    assert client.get("/api/v1/intelligence/tradeindia/report", headers=ORIGIN).status_code == 200


def test_tradeindia_performance_and_query_counts(client: TestClient) -> None:
    context = setup_context(client)
    client.post(
        "/api/v1/intelligence/tradeindia/discover",
    ...
    paths = (
        "/api/v1/intelligence/tradeindia/discoveries",
        "/api/v1/intelligence/tradeindia/operations",
...
def test_tradeindia_operations_doctor_and_calendar_are_truthful(client: TestClient) -> None:
    setup_context(client)
    operations = client.get("/api/v1/intelligence/tradeindia/operations", headers=ORIGIN).json()
    assert operations["provider"] == "TRADEINDIA" and operations["read_only"] is True
    ...
    assert client.get("/api/v1/intelligence/tradeindia/calendar", headers=ORIGIN).status_code == 200
    doctor = client.get("/api/v1/intelligence/system-doctor", headers=ORIGIN).json()
    assert doctor["checks"]["tradeindia"]["provider"] == "TRADEINDIA"


def test_tradeindia_provider_preflight_is_fail_closed() -> None:
    ...
    assert (
        provider_preflight(
            Settings(
                ...
                tradeindia_token_ref="configured",
            )
        )["status"]
        == "NOT_CONFIGURED"
    )
    assert provider_preflight(Settings(tradeindia_mode="DISABLED"))["network_call"] is False
    assert provider_preflight(Settings(tradeindia_mode="LOCAL_FIXTURE"))["network_call"] is False
    assert provider_preflight(Settings(tradeindia_mode="LIVE_READ_ONLY"))["network_call"] is False


def test_tradeindia_handoff_and_recovery_audit_are_idempotent(client: TestClient) -> None:
    context = setup_context(client)
    result = client.post(
        "/api/v1/intelligence/tradeindia/discover",
        json={
