...
def test_meesho_account_lifecycle(client: TestClient) -> None:
    setup = client.post(
        "/api/v1/auth/setup-owner",
        json={
            "full_name": "Meesho Owner",
            "email": "meesho-owner@example.com",
    ...
    created = client.post(
        ...
        json={
            "display_name": "Meesho Sandbox",
            "seller_account_id": "seller-meesho",
            "credentials": {},
        },
        headers=ORIGIN,
    )
    ...
    imported = client.post(
        f"/api/v1/marketplaces/meesho/accounts/{account_id}/orders/import",
        headers=ORIGIN,
    )
    ...
    financials = client.post(
        f"/api/v1/marketplaces/meesho/accounts/{account_id}/financial-events/import",
        headers=ORIGIN,
    )
    ...
    orders = client.get(f"/api/v1/marketplaces/meesho/accounts/{account_id}/orders", headers=ORIGIN)
    assert orders.status_code == 200, orders.text
    assert orders.json()[0]["fulfilments"][0]["remote_fulfilment_id"] == "MS-FULFILMENT-001"
    records = client.get(
        f"/api/v1/marketplaces/meesho/accounts/{account_id}/returns/records",
        headers=ORIGIN,
    )
    assert records.status_code == 200, records.text
    assert records.json()[0]["refunds"][0]["currency"] == "INR"
    profitability = client.get(
        f"/api/v1/marketplaces/meesho/accounts/{account_id}/profitability",
        headers=ORIGIN,
    )
