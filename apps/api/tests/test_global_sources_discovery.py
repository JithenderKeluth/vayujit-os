...
from vayujit_api.intelligence.global_sources import discover_local, provider_preflight


def test_global_sources_provider_defaults_disabled_and_fixture_is_deterministic() -> None:
    settings = Settings()
    readiness = provider_preflight(settings)
    assert readiness["provider"] == "GLOBAL_SOURCES"
...
def test_global_sources_live_mode_is_fail_closed() -> None:
    settings = Settings(
...
def test_global_sources_normalization_matrix_is_null_safe_and_deduplicated_by_runtime() -> None:
    cases = (
        "complete",
        "missing_price",
        "missing_currency",
    ...
    for case in cases:
        ...
        assert row.source_url.startswith("https://www.globalsources.com/")
