...
def test_fake_meesho_coherent_listing_lifecycle() -> None:
    reset_fake_meesho_state()
    connector = MeeshoCommerceConnector(seller_id="seller-meesho")
    assert connector.validate_account("seller-meesho")["valid"] is True
    category = connector.discover_categories()[0]["id"]
    variants = [{"stable_variant_key": "default", "sku": "MS-SKU-1", "price": "99"}]
    media = [
        {
            "position": 0,
    ...
    assert not MeeshoMediaPolicy.validate(media)
    assert not MeeshoVariantPolicy.validate(variants)
    assert not MeeshoPricingPolicy.validate(price, MEESHO_MARKETPLACE.currency)
    ...
    submitted = connector.submit(
        sku="MS-SKU-1", payload={"title": "Meesho product"}, idempotency_key="meesho-submit-1"
    )
    ...
    assert connector.get_inventory("MS-SKU-1")["quantity"] == 10
    assert connector.update_inventory("MS-SKU-1", 7, "inventory-1")["quantity"] == 7
    ...
    assert connector.get_inventory("MS-SKU-1")["quantity"] == 7
    assert connector.get_orders()[0]["remote_id"] == "MS-ORDER-001"
    assert Decimal(str(connector.get_financial_events()[0]["net"])) == Decimal("1125.00")
    assert connector.get_returns()[0]["status"] == "REQUESTED"


def test_meesho_endpoint_policy_rejects_unverified_urls() -> None:
    assert MeeshoEndpointPolicy.validate("https://seller.meesho.com") == "seller.meesho.com"
    for value in (
        "http://seller.meesho.com",
        "https://evil.example",
        "https://user:pass@seller.meesho.com",
    ):
        try:
            MeeshoEndpointPolicy.validate(value)
        except ValueError:
            pass
        else:
            raise AssertionError("unsafe Meesho endpoint was accepted")


def test_fake_transport_has_no_live_network_boundary() -> None:
    transport = FakeMeeshoTransport()
    assert transport.categories(search="home", offset=0, limit=10)[0]["id"] == "ms-home"


def test_fake_meesho_throttling_is_bounded_and_retryable() -> None:
    reset_fake_meesho_state()
    connector = MeeshoCommerceConnector(seller_id="seller-meesho")
...
def test_fake_meesho_ambiguous_mutation_is_reconciled() -> None:
    reset_fake_meesho_state()
    connector = MeeshoCommerceConnector(seller_id="seller-meesho")
    submitted = connector.submit(
        sku="MS-AMBIGUOUS-1",
