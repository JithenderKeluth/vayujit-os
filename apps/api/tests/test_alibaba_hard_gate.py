...
from vayujit_api.intelligence import alibaba_service
from vayujit_api.intelligence.alibaba import provider_preflight
from vayujit_api.intelligence.alibaba_models import AlibabaDiscoveryRequest
...
def test_alibaba_retry_matrix(case: str, code: str, expected: bool) -> None:
...
def test_alibaba_rate_and_retry_budget_matrix(client: TestClient) -> None:
    ...
    with test_ai_integration.factory() as db:
        ...
        available = consume_rate_window(
            db, owner, "ALIBABA", requests_per_minute=2, requests_per_hour=2
        )
        assert available.remaining == 1
        consume_rate_window(db, owner, "ALIBABA", requests_per_minute=2, requests_per_hour=2)
        ...
        row = MarketplaceExecution(
            owner_id=owner.id, provider="ALIBABA", correlation_id="budget", identity_key="budget"
        )
        db.add(row)
        db.flush()
...
def test_alibaba_crash_checkpoint_recovery_is_duplicate_free(
    ...
    with test_ai_integration.factory() as db:
        ...
        assert owner is not None
        calls = 0
        adapter = alibaba_service._AlibabaAdapter(
            Settings(alibaba_enabled=True, alibaba_mode="LOCAL_FIXTURE"),
            result_limit=2,
            country_code="CN",
        )
...
def test_alibaba_concurrency_and_repeatability(client: TestClient) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    ...
    def run_once(suffix: str) -> tuple[str, int]:
        ...
        def invoke(_: int) -> str:
            with factory() as db:
                ...
                assert owner is not None
                barrier.wait()
                result = alibaba_service.discover(
                    db,
                    owner,
                    Settings(alibaba_enabled=True, alibaba_mode="LOCAL_FIXTURE"),
        ...
        with factory() as db:
            count = (
                db.scalar(
                    ...
                    .where(AlibabaDiscoveryRequest.idempotency_key == f"concurrent-{suffix}")
                )
                or 0
            )
...
def _evidence_pair(
    ...
    right = AutonomousResearchEvidence(
        ...
        task_id=task.id,
        source_class="SUPPLIER",
        source_reference=f"alibaba-{key}",
        retrieval_identity=f"alibaba:{key}",
        content_type="application/json",
        ...
        freshness_status="FRESH",
        verification_reason="fixture",
        source_profile="alibaba-local",
        provider="ALIBABA",
        canonical_url="https://www.alibaba.com/source",
...
def test_alibaba_contradiction_matrix_is_replay_safe(client: TestClient, key: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_alibaba_change_matrix_and_replay(client: TestClient, field: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_alibaba_alert_matrix_and_replay(client: TestClient, alert_type: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
        ...
        first = record_external_alert(
            ...
            title=f"Alibaba {alert_type}",
            detail="Review required",
            identity=f"alibaba:{alert_type}",
        ...
        later = record_external_alert(
            ...
            identity=f"alibaba:{alert_type}:later",
            severity="REQUIRES_REVIEW",
        )
        db.commit()
...
def test_alibaba_rejected_data_has_zero_delta(client: TestClient, state: str) -> None:
    setup_context(client)
    assert test_ai_integration.factory is not None
    with test_ai_integration.factory() as db:
        owner = db.scalar(select(User).where(User.email == "owner@example.com"))
...
def test_alibaba_canonical_e2e_lineage_storage_and_replay(client: TestClient) -> None:
    context = setup_context(client)
    payload = {
        "query": "canonical alibaba bottle",
        "product_id": context["product"]["id"],
        "result_limit": 2,
        "idempotency_key": "alibaba-canonical",
    }
    ...
    with test_ai_integration.factory() as db:
        ...
        assert execution.provider == "ALIBABA" and execution.status == "SUCCEEDED"
        assert (
            execution.lineage
...
def test_alibaba_security_privacy_and_xss_boundaries(client: TestClient) -> None:
    context = setup_context(client)
    ...
    assert client.get("/api/v1/intelligence/alibaba/report", headers=ORIGIN).status_code == 200


def test_alibaba_performance_and_query_counts(client: TestClient) -> None:
    context = setup_context(client)
    client.post(
        "/api/v1/intelligence/alibaba/discover",
    ...
    paths = (
        "/api/v1/intelligence/alibaba/discoveries",
        "/api/v1/intelligence/alibaba/operations",
...
def test_alibaba_operations_doctor_and_calendar_are_truthful(client: TestClient) -> None:
    setup_context(client)
    operations = client.get("/api/v1/intelligence/alibaba/operations", headers=ORIGIN).json()
    assert operations["provider"] == "ALIBABA" and operations["read_only"] is True
    ...
    assert client.get("/api/v1/intelligence/alibaba/calendar", headers=ORIGIN).status_code == 200
    doctor = client.get("/api/v1/intelligence/system-doctor", headers=ORIGIN).json()
    assert doctor["checks"]["alibaba"]["provider"] == "ALIBABA"


def test_alibaba_provider_preflight_is_fail_closed() -> None:
    ...
    assert provider_preflight(Settings(alibaba_mode="DISABLED"))["network_call"] is False
    assert provider_preflight(Settings(alibaba_mode="LOCAL_FIXTURE"))["network_call"] is False
    assert provider_preflight(Settings(alibaba_mode="LIVE_READ_ONLY"))["network_call"] is False


def test_alibaba_handoff_and_recovery_audit_are_idempotent(client: TestClient) -> None:
    context = setup_context(client)
    result = client.post(
        "/api/v1/intelligence/alibaba/discover",
        json={
